...
Robot* EntityFactory::CreateRobot() {
  ...
  robot->set_radius(RobotRandomRadius());
  ++entity_count_;
  ++robot_count_;
...
int EntityFactory::RandomRadius() {
  // Funtion sets random radius foodd on range defined in params.h

  return (LIGHT_MIN_RADIUS + random() %
  ((LIGHT_MAX_RADIUS + 1) - LIGHT_MIN_RADIUS));
}

int EntityFactory::RobotRandomRadius() {
  return (ROBOT_MIN_RADIUS + random() %
  ((ROBOT_MAX_RADIUS + 1) - ROBOT_MIN_RADIUS));
}
