...
class GraphicsArenaViewer : public GraphicsApp {
 public:
   ...
   */
  /**
   * @brief Called if new game buton pressed to reset environment
   */
  void  NewGameBtnPressed();
  /**
   * @brief Called if to check current game status
   */
 ...
 private:
  ...
  Arena *arena_;
  bool paused_{true};
  bool new_game_{false};
  bool parameter_instantiation_flag_{true};
  bool food_button_active_ = false;
  // buttons
  nanogui::Button *playing_button_{nullptr};

  nanogui::Label *status_button_{nullptr};
